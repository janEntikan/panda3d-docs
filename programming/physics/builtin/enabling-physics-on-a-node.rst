...
base.physics_mgr, which will often be suitable for most applications.
...
.. only:: python
   ...
   .. code-block:: python
      ...
      anp = node.attach_new_node(an)
      base.physics_mgr.attach_physical_node(an)
      jetpack_guy = loader.load_model("models/jetpack_guy")
      jetpack_guy.reparent_to(anp)
...
.. only:: cpp
   ...
   .. code-block:: cpp
      ...
      NodePath jetpack_guy = window->load_model(anp, "models/jetpack_guy");

Now, the "jetpack_guy" model will be updated every frame with the physics
applied to it.
...
use the get_physics_object call.
